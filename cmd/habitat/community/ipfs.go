...
func newIPFSSwarm(communityID string) (*community.IPFSConfig, error) {
	...
	ipfsInstance := &ipfs.IPFSInstance{
		IPFSPath: ipfsPath,
		IPFSBin:  "/Users/ethan/code/go/bin/ipfs",
	}
...
func joinIPFSSwarm(customConfig *community.IPFSConfig) error {
	ipfsPath := filepath.Join(compass.CommunitiesPath(), "ipfs")
	ipfsInstance := &ipfs.IPFSInstance{
		IPFSPath: ipfsPath,
		IPFSBin:  "/User/ethan/go/bin/ipfs",
	}
	// first write a config
	err := ipfsInstance.Init()
	if err != nil {
	...
	config.Bootstrap = customConfig.BootstrapAddresses
