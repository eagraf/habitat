...
func (i *IPFSInstance) Init() error {
	binPath, err := exec.LookPath("ipfs")
	if err != nil {
		return err
	}
...
func (i *IPFSInstance) WriteSwarmKey(swarmKey []byte) error {
	err := ioutil.WriteFile(filepath.Join(i.IPFSPath, "swarm.key"), swarmKey, 0755)
	if err != nil {
		return err
	}
	return nil
...
func (i *IPFSInstance) Daemon() error {
	binPath, err := exec.LookPath("ipfs")
	if err != nil {
		return err
	}
	cmd := exec.Cmd{
		...
		Env: []string{
			fmt.Sprintf("IPFS_PATH=%s", i.IPFSPath),
		},
	}
