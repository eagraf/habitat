...
func errorWriter(w http.ResponseWriter, status int, msg string) {
	log.Error().Msg(msg)
...
func (rm *RaftMultiplexer) handler(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		log.Error().Msg("community_id missing from raft request parameters")
		w.WriteHeader(http.StatusBadRequest)
		w.Write([]byte("community_id missing from raft request parameters"))
	}

	if _, ok := rm.listeners[communityID]; !ok {
		log.Error().Msgf("community_id %s is not a listener", communityID)
		w.WriteHeader(http.StatusBadRequest)
		w.Write([]byte(fmt.Sprintf("community_id %s is not a listener", communityID)))
	}
	transport := rm.listeners[communityID]

	buf, err := ioutil.ReadAll(r.Body)
	if err != nil {
		log.Error().Msgf("unable to read req body: %s", err)
		w.WriteHeader(http.StatusBadRequest)
		w.Write([]byte(fmt.Sprintf("unable to read req body: %s", err)))
	}

	var reqBody HTTPTransportPostRequest
	err = json.Unmarshal(buf, &reqBody)
	if err != nil {
		log.Error().Msgf("unable to unmarshal req body: %s", err)
		w.WriteHeader(http.StatusBadRequest)
		w.Write([]byte(fmt.Sprintf("unable to unmarshal req body: %s", err)))
	}

	command, err := unmarshalRPCRequest(reqBody.RPCType, reqBody.Args)
	if err != nil {
		errorWriter(w, http.StatusBadRequest, fmt.Sprintf("error unmarshaling command: %s", err))
	}
...
RESP:
	...
	if resp.Error != nil {
		...
		errorWriter(w, http.StatusBadRequest, resp.Error.Error())
	}

	// Serialize raft response and serialize into HTTP response
	buf, err = json.Marshal(resp.Response)
	if err != nil {
		errorWriter(w, http.StatusInternalServerError, fmt.Sprintf("error marshaling raft response: %s", err))
	}
	...
	if err != nil {
		errorWriter(w, http.StatusInternalServerError, fmt.Sprintf("error marshaling http response body: %s", err))
	}
